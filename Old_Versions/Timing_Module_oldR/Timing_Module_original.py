...
def get_event_start_date_from_timing_descriptor(timing_descriptor):
    ...
    event_start_date_year = int("{}".format(timing_descriptor[-4:]))
    event_start_date_month = TimingMappings.loc[timing_period, ('Start', 'Month')]
    event_start_date_day = TimingMappings.loc[timing_period, ('Start', 'Day')]
    
    event_start_date = dt.date(event_start_date_year, event_start_date_month, event_start_date_day)
    return(event_start_date)


def event_prob_by_expiry(timing_descriptor, expiry):
    pass
#event_start_date_month = 

    dates = pd.date_range(pd.datetime.today(), periods = 100).tolist()

#    event_start_date = dt.datetime(
    
    

event_prob_by_expiry('Q1_2018', dt.date(2018, 12, 1))
