...
class ScrubParams(object):
    """Three Parameters for Scrubbing Process:
        ...
        - Percentile Cutoff: With the remaining data points, keep the n-percentile data points with the best fit on
            the OLS Regression.
        """
    def __init__(self,
                 stock_cutoff=5,
    ...
    def __repr__(self):
        return "ScrubParams(Stock Cutoff: {:2f}, Index Cutoff: {:2f}, Percentile Cutoff: {:2f})".format(
            self.stock_cutoff, self.index_cutoff, self.percentile_cutoff)
