...
from beta_model.scrubbing_processes import DEFAULT_STOCK_CEILING_PARAMS, DEFAULT_INDEX_FLOOR_PARAMS, BEST_FIT_PERCENTILE, get_scrub_params

# Paul Utils
from utility.decorators import my_time_decorator, empty_decorator
from utility.general import setup_standard_logger, merge_dfs_horizontally, append_dfs_vertically, to_pickle_and_CSV
from utility.finance import get_ETF_beta_to_SPY, get_total_return
...
def get_betas_multiple_stocks(stocks,
    ...
    beta_values= [beta_value.beta_value for beta_value in betas]
    corrs = [beta_value.corr for beta_value in betas]
    stock_cutoffs = [scrub_params.stock_cutoff for scrub_params in scrub_params_all]
    index_cutoffs = [scrub_params.index_cutoff for scrub_params in scrub_params_all]
    ...
    idio_returns = [(1+returns[i])/(1+index_returns[i]*beta_values[i]) - 1 for i in range(len(stocks))]
    percent_days_in_calc = [beta_value.percent_days_in_calculation for beta_value in betas]
    # Create DataFrame
...
def get_best_betas():
    ...
    for stock in stocks:
        ...
        for index in indices:
            ...
            beta_value = Beta(stock, index, lookback, scrub_params)
            outcomes.append((index, beta_value.beta_value, beta_value.corr))
