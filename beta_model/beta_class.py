...
from beta_model.scrubbing_processes import stock_ceiling_scrub_process, index_floor_scrub_process,\
    best_fit_scrub_process, DEFAULT_STOCK_CEILING_PARAMS, DEFAULT_INDEX_FLOOR_PARAMS,\
    BEST_FIT_PERCENTILE, get_scrub_params
...
class Beta(object):
    ...
    def initial_scrub(self):
        """Eliminate data points where the stock moved greater (absolute value) than the specified stock cutoff.
           (Said differently, only keep data points below the stock ceiling)
           This process imposes a ceiling for the magnitude of stock moves included in the sample."""
    ...
    # These calculations use my own OLS class as opposed to the smmodule.
    # The degrees are freedom differ by one (not sure how to affect this parameter with the open-source model.
    @property
    def OLS_object(self):
    ...
    def describe(self):
        # Previously used self.beta1, now using self.beta_value
        """Scrub Type: Beta, Corr., n"""
        pprint("{}-> Beta: {:.2f}, Corr.: {:.2f}, n = {:.0f}".format(
            self.scrub_type, self.beta_value, self.corr, self.degrees_of_freedom))
    ...
    def show_scrub_trajectory(self):
        ...
        third_scrub = ScrubParams(self.scrub_params.stock_cutoff, self.scrub_params.index_cutoff,
                                  self.scrub_params.percentile_cutoff)
