from beta_model.ols_df_transform import create_ols_df
from utility.decorators import my_time_decorator, empty_decorator
from utility.finance import get_daily_returns, calculate_percentile_value, calculate_average_daily_move_from_returns,\
    calculate_SD_multiple_from_returns
...
# The big difference between the two types of scrubbing is that in the original way, I delete the rows from the columns.
# In the second way, I show a NaN value. Which way is better?
...
def stock_ceiling_scrub_process(returns_df_to_scrub: 'DataFrame of daily returns (multiple stocks allowed)',
    ...
    if stock_move_ceiling:
        ...
        if stock_to_drive_scrubbing == False:
            stock_to_drive_scrubbing = returns_df_to_scrub.columns[0]
    
        mask = abs(returns_df_to_scrub[stock_to_drive_scrubbing]) <= stock_move_ceiling
        scrubbed_df = returns_df_to_scrub[mask]
        return scrubbed_df
...
# I don't think this is used anywhere in the project. Defaults are in all caps above.
...
def determine_stock_ceiling_for_scrub_process_from_stock_returns(returns_df,
    ...
    stock_ceiling = min(stock_ceiling, stock_ceiling_params.initial_ceiling)
...
def get_scrub_params(stock,
    ...
    stock_ceiling = determine_stock_ceiling_for_scrub_process(stock, lookback, stock_ceiling_params)
    index_floor = determine_index_floor_for_scrub_process(index, lookback, index_floor_params)
